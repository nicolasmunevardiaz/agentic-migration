# NorthCare Clinics Privacy And Governance Review
...
- Direct identifiers: member names and `SSN_NUM`.
...
- Confirm that `OBS_PAYLOAD` remains preserved as source text until canonical modeling.
- Confirm financial handling for `MED_UNIT_PRICE` before Silver semantics.
- Confirm that X12-style parsing is the approved source contract despite the dictionary's CSV wording.
