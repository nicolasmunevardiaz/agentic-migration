...
The first skill set should remain small, but each skill must have a concrete output. The highest-risk skill is `provider-spec-generator`; it must not stop at a report. Its DoD is a set of YAML provider/entity specs that can later drive adapter implementation.
...
|---|---|---|---|
| `provider-spec-generator` | Expert parsing and source profiling skill that reads provider folders and `column_dictionary.md`, detects file/entity structure, standardizes metadata, and emits declarative YAML specs | Provider folders, `column_dictionary.md`, raw file paths, topology PRD | `metadata/provider_specs/<provider_slug>/<entity>.yaml`, provider drift summary, human-review questions |
