...
| Local data workbench and model evolution | Sequential, all approved local runtime artifacts | Prompt 4.5 |
...
## Prompt 4.5: Local Data Workbench And Model Evolution

Explanation: This prompt inserts the local PostgreSQL/dbt model evolution stage after local runtime certification and before Databricks rollout. It governs iterative Silver normalization, local workbench loading, drift resolution, business-question registry versions, SQL outputs, model snapshots, rollback, and robust terminal-driven QA without using Databricks or production data.

Prerequisites: Plan 04 local runtime certification PR is approved or merged. Local PostgreSQL exists and is reachable from terminal. Human has approved local `uv`, PostgreSQL usage, and any already-installed local-only packages. dbt Core may be used only after its local project/profile layout is declared. No Databricks job, Terraform, bundle deploy, Docker Desktop, production data, or cloud resource use is approved.

```text
You are Codex executing Local Data Workbench And Model Evolution across all approved providers.

Use docs/04_5_local_data_workbench_and_model_evolution_plan.md as the active plan. Read docs/technical_prd_agentops_operating_spec.md, docs/agentops_filesystem_conventions.md, docs/agentops_skill_strategy.md, docs/local_postgres_workbench_deploy.md, docs/local_runtime_macos_requirements.md, metadata/provider_specs/**, metadata/model_specs/bronze/bronze_contract.yaml, metadata/model_specs/silver/**, metadata/model_specs/mappings/provider_to_silver_matrix.yaml, metadata/model_specs/impact/business_question_profiles.yaml, metadata/model_specs/evolution/**, metadata/runtime_specs/local/**, src/**, tests/**, reports/hitl/**, reports/privacy/**, reports/qa/**, logs/local_runtime/**, and local artifacts only when they are generated by approved local validation.

Use these skills: local-postgres-dbt-modeling-orchestrator, local-runtime-harness-planner, qa-evidence-reviewer, privacy-governance-reviewer, spec-test-generator, hitl-escalation-controller, and repo-governance-auditor before PR creation.

Work as a local PostgreSQL/dbt model evolution architect. Treat metadata/model_specs/impact/business_question_profiles.yaml as a living contract. Every material normalization change must update the active model snapshot under metadata/model_specs/evolution/V0_N/ and record the exact business-question registry version BQ_V0_N with checksum, question count, field-decision count, completion status, and acceptance rule. Do not hand-patch PostgreSQL as durable model history.

Use .agent/spec_templates/model_iteration_packet.template.yaml for iteration handoffs, .agent/spec_templates/normalization_probe.template.yaml for exploratory normalization, and .agent/spec_templates/semantic_discovery_issue.template.yaml only when ambiguity or risk remains. If you find nested data inside columns, data that belongs to a different entity, mixed grain, unsafe casts, duplicate keys, orphan relationships, unit/code ambiguity, temporal ambiguity, sparse unmapped fields, or PII/PHI exposure, first decompose the problem into a small normalization probe. Use temp tables or scratch schema transition tables to test candidate normalization, measure results, and promote only when metadata, tests, and rollback are updated. Stop for HITL only when ownership, meaning, privacy, clinical interpretation, financial interpretation, or business impact remains ambiguous after the probe or cannot be safely probed.

Use PostgreSQL as the local HITL read/write workbench and dbt Core as the preferred local SQL orchestration/testing tool only after the dbt layout is declared. Iterate from approved Silver contracts toward tested SQL outputs that answer every business question, or explicitly record HITL-approved deferrals. Local Gold candidates may be introduced only when derived from resolved business questions and protected by tests; do not create production serving models or claim Databricks parity.

Generate or update model evolution snapshots under metadata/model_specs/evolution/V0_N/, PostgreSQL DDL images, local deploy/runbook docs, dbt readiness artifacts when approved, runtime/deploy handlers under src/ only after specs exist, and tests under tests/specs plus future tests/dbt or tests/sql when needed. Append concise trace entries to logs/local_runtime/local_runtime_certification.md.

Run validation with uv. Required gates include tests/specs, model evolution registry tests, PostgreSQL dry-run/apply/verify when HITL allows local DB writes, adapter tests when data loading changes, ruff check, and SQL/dbt tests when dbt models are introduced. If business-question evidence, dependency approvals, HITL decisions, rollback target, SQL answer tests, or local DB access are missing, invoke hitl-escalation-controller and stop.

When local model evolution QA passes, run repo-governance-auditor. If it returns allowed_next_action: create_pr, use safe gh CLI to create a PR against main with title "model: local-workbench - add model evolution snapshot". Include plan id, provider=all, skills, changed files, tests, evidence, business-question registry version, SQL-output status, HITL decisions, rollback snapshot, risks, Databricks impact, and branch deletion notes.
```
...
Prerequisites: Provider, canonical, adapter, local runtime certification, and local model evolution PRs are approved or merged, or Plan 04.5 has an explicit HITL-approved deferral. GitHub CI has passed. Human has created required Databricks secrets only for approved validation environments and has explicitly approved Databricks readiness planning.
