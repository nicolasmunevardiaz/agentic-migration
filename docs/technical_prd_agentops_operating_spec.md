...
### 3.2.1 Local Model Evolution Plane: PostgreSQL/dbt Before Gold Promotion

After Plan 04 local runtime certification, Plan 04.5 introduces a local model evolution plane. Its responsibility is to iterate from approved Silver contracts toward tested SQL answers for every business question in `metadata/model_specs/impact/business_question_profiles.yaml`.

This plane uses PostgreSQL as the local read/write workbench and HITL decision interface. dbt Core is the preferred local SQL orchestration and test mechanism when its project layout and profile are approved. The plane may create local Gold candidates only after Silver normalization decisions are resolved, but it must not treat those candidates as production Databricks serving models.

Every material iteration must create a complete model snapshot under `metadata/model_specs/evolution/V0_N/`. The snapshot must identify the exact `business_question_profiles.yaml` registry version it implements, include full PostgreSQL DDL, define rollback to a previous complete snapshot, and list unit, integration, regression, and e2e validation commands. Rollback means redeploying a previous approved snapshot, not hand-editing database tables.
...
| Business Question Profiles | Reverse-engineer business intent into minimum canonical concepts, provider evidence, provider/entity/field decision records, risk-ranked solution options, HITL requests, and deferred Gold candidates before canonical modeling | `business-request.md`, `.agent/spec_templates/business_question_profile.template.yaml`, provider specs, drift reports, privacy reports, HITL queues, QA evidence | `metadata/model_specs/impact/business_question_profiles.yaml`, field-level HITL decision requests, Plan 02 modeling inputs | `business-question-profiler` |
...
| Canonical Model Specs | Convert provider source evidence and business profiling into shared Bronze/Silver contracts, canonical entities, types, required fields, lineage, keys, mappings, and modeling risks | Provider specs, business question profiles, topology PRD, governance requirements, drift decision runbook | `metadata/model_specs/bronze/bronze_contract.yaml`, `metadata/model_specs/silver/<entity>.yaml`, `metadata/model_specs/mappings/provider_to_silver_matrix.yaml`, `metadata/model_specs/impact/modeling_risk_report.md` | `canonical-model-planner` |
| Local Model Evolution Snapshots | Version PostgreSQL/dbt model iterations, business-question registry versions, full DDL images, rollback targets, and SQL-output readiness from Silver toward local Gold candidates | Bronze/Silver specs, provider-to-Silver matrix, business-question profiles, local runtime specs, PostgreSQL/dbt evidence | `metadata/model_specs/evolution/V0_N/model_snapshot.yaml`, `metadata/model_specs/evolution/V0_N/postgres_schema.sql`, later dbt and business-question SQL artifacts when approved | `local-postgres-dbt-modeling-orchestrator` |
...
| Local Model Evolution Snapshots | Schema tests, integration tests, regression tests, local e2e tests, SQL-output tests | Snapshot completeness, business-question registry version, full PostgreSQL DDL image, rollback target, no manual patches, dbt/PostgreSQL alignment, tested SQL for resolved business questions | `tests/specs/test_model_specs.py`, future `tests/dbt/` or `tests/sql/` |
...
The initial AgentOps skill set should be small, precise, and complete enough to move from specs into implementation. The recommended v1 skills are `provider-spec-generator`, `drift-decision-resolver`, `canonical-model-planner`, `adapter-contract-reviewer`, `adapter-code-generator`, `local-runtime-harness-planner`, `local-postgres-dbt-modeling-orchestrator`, `spec-test-generator`, `privacy-governance-reviewer`, `qa-evidence-reviewer`, `repo-governance-auditor`, `hitl-escalation-controller`, and `databricks-rollout-planner`.
...
Plan 04.5 acceptance is stricter than Plan 04 acceptance. Plan 04.5 is complete only when every business question in `metadata/model_specs/impact/business_question_profiles.yaml` has a tested local SQL output or an explicit HITL-approved deferral; the active model snapshot records the exact business-question registry version; PostgreSQL/dbt validation covers unit, integration, regression, and e2e behavior; rollback to the previous approved snapshot is defined and tested; and no production, Databricks, Terraform, bundle, Docker Desktop, or unapproved serving-model action is introduced.
