...
When local `data_500k/<provider>/year=2025/` source data is present, Plan 03 adapter integration tests must run every file that matches the active provider/entity spec pattern, not a handpicked sample. The only allowed skip is when the entire local source dataset is absent from the environment; do not skip individual corrupt files or weaken tests to pass. Parser/export wrapper tolerances, such as provider comment lines, file preambles, trailers, fallback encodings, or copy-load style options, must be declared in provider specs or shared parser logic. Malformed files, unsupported profiles, and load failures must surface provider, entity, source file path, checksum when available, error type, and decision in pytest output or QA evidence so the issue can be fixed before Silver certification. The behavior should be explicit like database load policies: stop on malformed file contracts, warn for nullable canonical value issues, and quarantine only when an approved quarantine rule says so.

When any local `data_500k` provider dataset exists, Plan 03 QA must also run the reusable file-level audit command without provider filtering: `uv run --no-sync python -m src.handlers.data_500k_adapter_audit`. The command writes `artifacts/qa/data_500k_adapter_load_audit.jsonl` and `artifacts/qa/data_500k_adapter_load_audit.md`. The default run must cover every registered adapter audit target, every default entity, and every source file matching provider spec patterns. Provider-filtered runs are allowed only for focused diagnosis; they are not complete PR evidence. The JSONL audit is the source of truth for local `data_500k` pass/fail/skip evidence and must remain generated programmatically, not manually edited.

Expected local commands are `uv run pytest tests/specs`, `uv run pytest tests/adapters`, `uv run --no-sync python -m src.handlers.data_500k_adapter_audit`, `uv run pytest tests/fixtures`, `uv run ruff check`, and `uv run python -m <approved_validation_module>` once such a module exists.
