...
def landing_tables(spec: dict[str, Any], repo_root: Path) -> list[Table]:
    landing_config = spec["landing_tables"]
    silver_root = repo_root / landing_config["source_spec_root"]
    additional_columns = tuple(
        column_from_mapping(column, TYPE_MAP)
        for column in landing_config.get("additional_columns", [])
    )
    tables: list[Table] = []
    for silver_path in sorted(silver_root.glob("*.yaml")):
        ...
        tables.append(
            Table(
                ...
                name=f"{landing_config['table_prefix']}{entity}",
                columns=columns + additional_columns,
                indexes=(
                    Index(
                        name=f"idx_{landing_config['schema']}_{landing_config['table_prefix']}{entity}_lineage",
...
def all_tables(spec: dict[str, Any], repo_root: Path) -> list[Table]:
    ...
    if spec["landing_tables"]["enabled"]:
        tables.extend(landing_tables(spec, repo_root))
    return tables
