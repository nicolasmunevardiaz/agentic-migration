...
Read provider specs, model specs, QA evidence contracts, Databricks deployment specs, dependency manifests, lockfiles, PR risk reports, and any generated fixtures or examples. Confirm that PII/PHI fields are flagged, sensitive examples are redacted or synthetic, lineage and audit fields are present, Unity Catalog targets are governed, permissions are explicit enough for review, and dependencies are justified and supportable.

Validate that secrets are not committed, local absolute paths are not embedded, QA evidence does not leak raw sensitive values, and approval records exist for PII classification, masking/redaction expectations, ambiguous identifiers, Silver required fields, quarantine rules, Databricks execution, and dependency risk exceptions.
...
Do not downgrade PII/PHI classification, approve semantic mappings, grant permissions, authorize cloud execution, or approve risky dependencies without evidence. Do not allow privacy, governance, or dependency risk decisions to be hidden inside adapter logic. Governance decisions must be visible in specs, QA evidence, Human in the Loop records, and concise append-only trace logs.
