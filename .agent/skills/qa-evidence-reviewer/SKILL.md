...
When QA evidence says validation passed, verify that the report clearly states what passed and what did not. Passing parser/spec tests means the repository artifact is internally consistent and PR-ready; it does not mean mappings, PII/PHI handling, status normalization, relationship confidence, Databricks execution, or adapter readiness have been approved by a human. Require reports to keep those two meanings separate.

Do not replace descriptive test names with opaque rule IDs. Do not allow failed checks to disappear into logs. Do not let model-graded review replace deterministic parser, adapter, schema, data quality, or reconciliation checks.
