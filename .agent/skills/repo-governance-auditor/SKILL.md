...
The only long-lived branch is `main`. Agent branches are temporary work branches and must be scoped, for example `agentops/<plan-id>/<provider-or-scope>` or `governance/<scope>`, when branch creation is explicitly authorized. Agent-created PRs must target `main`. If a PR targets anything other than `main`, require an explicit Human in the Loop exception recorded in the PR evidence and return a blocked status unless that exception exists.
