...
def test_aegis_adapter_quarantines_invalid_required_decimal(tmp_path: Path) -> None:
    ...
    assert "cost_records" not in result.silver_rows
    assert [record.column_name for record in result.quarantine_records] == ["cost_amount"]
    assert result.quarantine_records[0].decision == "quarantine_data"
