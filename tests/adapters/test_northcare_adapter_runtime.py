...
def test_northcare_adapter_emits_bronze_and_canonical_silver_rows(entity: str) -> None:
    result = run_northcare_adapter_for_file(
        entity=entity,
        source_file=FIXTURE_ROOT / f"{entity}_message.txt",
    ...
    assert bronze.provider_slug == "data_provider_3_northcare_clinics"
    assert bronze.source_entity == entity
    assert bronze.source_row_key_header == "EXPORT_ID"
...
def test_northcare_adapter_uses_model_specs_for_silver_column_names() -> None:
    result = run_northcare_adapter_for_file(
        entity="conditions",
        source_file=FIXTURE_ROOT / "conditions_message.txt",
        ingestion_run_id="test-run-001",
        provider_spec_root=SPEC_ROOT,
        model_root=MODEL_ROOT,
    )
    ...
    assert "PT_REF" not in condition_row


def test_northcare_adapter_derives_observation_values_from_approved_payload_contract() -> None:
    result = run_northcare_adapter_for_file(
        entity="observations",
        source_file=FIXTURE_ROOT / "observations_message.txt",
        ingestion_run_id="test-run-001",
        provider_spec_root=SPEC_ROOT,
        model_root=MODEL_ROOT,
    )
...
def test_northcare_adapter_warns_on_invalid_ingestion_decimal(tmp_path: Path) -> None:
    invalid_medication = tmp_path / "medications_message.txt"
    text = (FIXTURE_ROOT / "medications_message.txt").read_text(encoding="utf-8")
    invalid_medication.write_text(
        text.replace("*12.34*2025-02-01*", "*not-decimal*2025-02-01*"),
        encoding="utf-8",
    )
...
def test_northcare_adapter_warns_on_malformed_optional_observation_payload(
    tmp_path: Path,
) -> None:
    invalid_observation = tmp_path / "observations_message.txt"
    text = (FIXTURE_ROOT / "observations_message.txt").read_text(encoding="utf-8")
...
def test_northcare_adapter_evidence_does_not_include_raw_sensitive_values() -> None:
    result = run_northcare_adapter_for_file(
        entity="patients",
        source_file=FIXTURE_ROOT / "patients_message.txt",
        ingestion_run_id="test-run-001",
        provider_spec_root=SPEC_ROOT,
        model_root=MODEL_ROOT,
    )
...
@pytest.mark.parametrize("entity", sorted(EXPECTED_SILVER_ENTITIES))
def test_northcare_adapter_runs_against_local_data_500k_files(entity: str) -> None:
    sample_files = sorted((DATA_ROOT / entity).glob(f"{entity}_*.txt"))
    if not sample_files:
        pytest.skip("data_500k local NorthCare dataset is not present in this environment")
