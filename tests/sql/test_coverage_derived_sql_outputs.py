...
def test_coverage_derived_models_are_declared_in_derived_schema() -> None:
    ...
    assert {model["name"] for model in schema["models"]} == {
        ...
        "coverage_period_survivor_candidate",
        "coverage_period_survivor_fact",
...
def test_coverage_derived_sql_preserves_period_grain_and_quality_flags() -> None:
    sql_text = "\n".join(
        ...
        for path in sorted((DBT_ROOT / "models/derived/coverage").rglob("*.sql"))
    )

    assert "{{ source('landing', 'coverage_periods') }}" in sql_text
    ...
    assert "survivor_rank" in sql_text
    assert "latest_loaded_record_per_provider_member_period_status" in sql_text
...
def test_coverage_v0_5_records_coverage_evidence() -> None:
    ...
    assert db_state["coverage_quality_summary"]["source_relation"] in {
