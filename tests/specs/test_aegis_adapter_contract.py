...
def test_aegis_canonical_mappings_reference_existing_provider_fields() -> None:
    provider_fields = {
        path.stem: {
            field["source_header"]
...
def test_aegis_adapter_contract_does_not_promote_direct_identifier_columns() -> None:
    silver_columns = {
        column["name"]
        for silver_path in SILVER_ROOT.glob("*.yaml")
