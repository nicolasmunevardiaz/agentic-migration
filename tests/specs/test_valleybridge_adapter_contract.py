...
def test_valleybridge_canonical_mappings_reference_existing_provider_fields() -> None:
    provider_fields = {
        path.stem: {
            field["source_header"]
        ...
        for column in silver_spec["columns"]:
            for mapping in column["source_mappings"]:
                ...
                assert (
                    ...
                    in provider_fields[mapping["source_entity"]]
                ), mapping

    assert mapped_source_entities == EXPECTED_SOURCE_ENTITIES


def test_valleybridge_adapter_contract_does_not_promote_direct_identifier_columns() -> None:
    silver_columns = {
        column["name"]
        for silver_path in SILVER_ROOT.glob("*.yaml")
